/* Process model C++ form file: EDAppLayerOld.pr.cpp */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char EDAppLayerOld_pr_cpp [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 5BCF6D76 5BCF6D76 1 DESKTOP-L8S8FMO Mohammed@Hussein 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                       ";
#include <string.h>
...
#include "headers\CsocketKafka.h"
using namespace std;
...
class EDAppLayerOld_state
	...
	public:
		...
		/* FSM code */
		void EDAppLayerOld (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_EDAppLayerOld_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype EDAppLayerOld_state::obtype = (VosT_Obtype)OPC_NIL;

#define pr_state_ptr            		((EDAppLayerOld_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
#define FIN_PREAMBLE_DEC	EDAppLayerOld_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((EDAppLayerOld_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
//	header_size = (int) op_pk_total_size_get(packet_MSDU_ptr);
		/////////////////////////////////////////////////////////////////////////////
	//char Data[100];i++;
	int temperature = rand() % 30; /* random int between 0 and 30 */ 	
	...
	Packet*	RfidPkptr= op_pk_create_fmt("rfid_hbi_data_packet");
     ...
     op_pk_nfd_set (RfidPkptr, "productID",ListOfRFID[Lsize]->productID);
     op_pk_nfd_set (RfidPkptr, "RfidValue1",ListOfRFID[Lsize]->RfidValue1);
	if (Lsize<ListOfRFID.size()-1) Lsize++;
	//op_pk_nfd_set (RfidPkptr, "RFID ID",22);
  //  op_pk_nfd_set (RfidPkptr, "productID",33);
   // op_pk_nfd_set (RfidPkptr, "RfidValue1",55);
	
	//op_pk_nfd_set (RfidPkptr, "RfidValue2",ListOfRFID[sendDATA]->RfidValue2);
	//op_pk_nfd_set (RfidPkptr, "RfidValue3",ListOfRFID[sendDATA]->RfidValue3);
	op_pk_nfd_set (RfidPkptr, "Temperature",temperature);
	op_pk_nfd_set (RfidPkptr, "Humidity",humidity );
	op_pk_nfd_set (RfidPkptr, "CO2",co2);
	//op_pk_nfd_set (RfidPkptr, "Address",my_address);
...
//	double packet_NSDU_size; 
	double e2e_delay; 
 	int TagID, productID,RfidValue1;
	int traffic_type;
	//char Data[100];
	char sendThisMsgToKafka[1000];// here it was the problem
	int temperature,humidity,co2 ;
	/* Stack tracing enrty point */
...
//	packet_NSDU_size = (double) op_pk_total_size_get (packet_NSDU_ptr);
	...
	op_stat_write (pktssec_rcvd_gstathandle, 	0.0);
 	...
 	op_pk_nfd_get(packet_MSDU_ptr, "productID", &productID);
	op_pk_nfd_get(packet_MSDU_ptr, "RfidValue1", &RfidValue1);
	//op_pk_nfd_get(packet_MSDU_ptr, "RfidValue2", &RfidValue2);
	//op_pk_nfd_get(packet_MSDU_ptr, "RfidValue3", &RfidValue3);
	op_pk_nfd_get(packet_MSDU_ptr, "Temperature", &temperature);
	op_pk_nfd_get(packet_MSDU_ptr, "Humidity", &humidity);
	op_pk_nfd_get(packet_MSDU_ptr, "CO2", &co2);
	//op_pk_nfd_get(packet_MSDU_ptr, "Address", &RfidSrcAddress);
 ...
 sprintf (sendThisMsgToKafka, "Time=%f , Temperature(%d), Humidity(%d) Co2=%d TagID=%d productID=%d RfidValue1=%d   MessageNumber:(%d) to Kafka ",		op_sim_time(),temperature,humidity,co2,TagID,productID,RfidValue1,  DATArecieved  );		
	// kafka
	...
	cout<<"This is the recieved pck from Destination and Number ="<<DATArecieved<<endl;
	/* Stack tracing exit point */
	FOUT;
}
...
extern "C"
{
	VosT_Obtype _op_EDAppLayerOld_init (int * init_block_ptr);
	VosT_Address _op_EDAppLayerOld_alloc (VosT_Obtype, int);
	void EDAppLayerOld (OP_SIM_CONTEXT_ARG_OPT)
		{
		((EDAppLayerOld_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->EDAppLayerOld (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_EDAppLayerOld_svar (void *, const char *, void **);

	void _op_EDAppLayerOld_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((EDAppLayerOld_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_EDAppLayerOld_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_EDAppLayerOld_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (EDAppLayerOld_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
EDAppLayerOld_state::EDAppLayerOld (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (EDAppLayerOld_state::EDAppLayerOld ());
	try
		{
		/* Temporary Variables */
		Packet*		RfidPkptr;
		...
		FSM_ENTER ("EDAppLayerOld")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "EDAppLayerOld [init enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayerOld [init enter execs]", state0_enter_exec)
				{
				/* Initialization */
				wpan_appl_init ();
			...
			FSM_STATE_EXIT_FORCED (0, "init", "EDAppLayerOld [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "EDAppLayerOld [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "EDAppLayerOld [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"EDAppLayerOld")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "EDAppLayerOld [idle exit execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayerOld [idle exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("EDAppLayerOld [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "SEND_BE", "", "idle", "send BE", "send best-effort packet", "EDAppLayerOld [idle -> send BE : SEND_BE / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "SEND_RT", "", "idle", "send RT", "send real-time packet", "EDAppLayerOld [idle -> send RT : SEND_RT / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "RECEIVE_PCK", "", "idle", "receive pck", "receive packet from NWK", "EDAppLayerOld [idle -> receive pck : RECEIVE_PCK / ]")
				FSM_CASE_TRANSIT (3, 2, state2_enter_exec, ;, "START_BE_APP", "", "idle", "send BE", "tr_14", "EDAppLayerOld [idle -> send BE : START_BE_APP / ]")
				FSM_CASE_TRANSIT (4, 5, state5_enter_exec, ;, "ReceiveRFID", "", "idle", "receive RFID", "tr_16", "EDAppLayerOld [idle -> receive RFID : ReceiveRFID / ]")
				FSM_CASE_TRANSIT (5, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_13", "EDAppLayerOld [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "send BE", state2_enter_exec, "EDAppLayerOld [send BE enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayerOld [send BE enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "send BE", "EDAppLayerOld [send BE exit execs]")


			/** state (send BE) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send BE", "idle", "tr_6", "EDAppLayerOld [send BE -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send RT) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send RT", state3_enter_exec, "EDAppLayerOld [send RT enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayerOld [send RT enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send RT", "EDAppLayerOld [send RT exit execs]")


			/** state (send RT) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send RT", "idle", "tr_8", "EDAppLayerOld [send RT -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (receive pck) enter executives **/
			FSM_STATE_ENTER_FORCED (4, "receive pck", state4_enter_exec, "EDAppLayerOld [receive pck enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayerOld [receive pck enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "receive pck", "EDAppLayerOld [receive pck exit execs]")


			/** state (receive pck) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "receive pck", "idle", "tr_10", "EDAppLayerOld [receive pck -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (receive RFID) enter executives **/
			FSM_STATE_ENTER_FORCED (5, "receive RFID", state5_enter_exec, "EDAppLayerOld [receive RFID enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayerOld [receive RFID enter execs]", state5_enter_exec)
				{
				//if (RFID_log)
					{
				...
				int TagID, RFIDvalue1,RFIDvalue2,RFIDvalue3, productID;
				
				RfidPkptr = op_pk_get (op_intrpt_strm ());
				//if (op_pk_nfd_get_ptr(RfidPkptr, "FlowTables", (void**) &flowEntery) == OPC_COMPCODE_SUCCESS)
				op_pk_nfd_get(RfidPkptr, "ProductID", &productID);
				op_pk_nfd_get(RfidPkptr, "TagID", &TagID);
				op_pk_nfd_get(RfidPkptr, "RFID value1", &RFIDvalue1);
				op_pk_nfd_get(RfidPkptr, "RFID value2", &RFIDvalue2);
				op_pk_nfd_get(RfidPkptr, "RFID value3", &RFIDvalue3);
				
				
				fprintf(Applog_file,"Reader sent ProductID: %d**\n",productID);
				fprintf(Applog_file,"Reader sent TagID: %d**\n",TagID);
				fprintf(Applog_file,"Reader sent RFID value1: %d**\n",RFIDvalue1);
				fprintf(Applog_file,"Reader sent RFID value2: %d**\n",RFIDvalue2);
				fprintf(Applog_file,"Reader sent RFID value3: %d**\n",RFIDvalue3);
				
				
				RFIDList* Rfidlist = new RFIDList;
				
				Rfidlist->RfidID=TagID;
				Rfidlist->productID = productID;
				Rfidlist->RfidValue1=RFIDvalue1;
				Rfidlist->RfidValue2=RFIDvalue2;
				Rfidlist->RfidValue3=RFIDvalue3;
				...
				fprintf (Applog_file, "TagID=%d & productID== %d & RfidValue1=%d & RfidValue2=%d RfidValue3=%d \n",
					ListOfRFID[c]->RfidID,ListOfRFID[c]->productID,ListOfRFID[c]->RfidValue1,ListOfRFID[c]->RfidValue2,ListOfRFID[c]->RfidValue3 );
			...
			FSM_STATE_EXIT_FORCED (5, "receive RFID", "EDAppLayerOld [receive RFID exit execs]")


			/** state (receive RFID) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "receive RFID", "idle", "tr_15", "EDAppLayerOld [receive RFID -> idle : default / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"EDAppLayerOld")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (EDAppLayerOld)",
...
void
EDAppLayerOld_state::_op_EDAppLayerOld_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}

void
EDAppLayerOld_state::operator delete (void* ptr)
	{
	FIN (EDAppLayerOld_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

EDAppLayerOld_state::~EDAppLayerOld_state (void)
	{

	FIN (EDAppLayerOld_state::~EDAppLayerOld_state ())
...
void *
EDAppLayerOld_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (EDAppLayerOld_state::operator new ());

	new_ptr = Vos_Alloc_Object (EDAppLayerOld_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
EDAppLayerOld_state::EDAppLayerOld_state (void) :
		_op_current_block (0)
	{
		...
		_op_current_state = "EDAppLayerOld [init enter execs]";
#endif
	}

VosT_Obtype
_op_EDAppLayerOld_init (int * init_block_ptr)
	{
	FIN_MT (_op_EDAppLayerOld_init (init_block_ptr))

	EDAppLayerOld_state::obtype = Vos_Define_Object_Prstate ("proc state vars (EDAppLayerOld)",
		sizeof (EDAppLayerOld_state));
	*init_block_ptr = 0;

	FRET (EDAppLayerOld_state::obtype)
	}

VosT_Address
_op_EDAppLayerOld_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	EDAppLayerOld_state * ptr;
	FIN_MT (_op_EDAppLayerOld_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new EDAppLayerOld_state;
...
void
_op_EDAppLayerOld_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	EDAppLayerOld_state		*prs_ptr;

	FIN_MT (_op_EDAppLayerOld_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (EDAppLayerOld_state *)gen_ptr;
