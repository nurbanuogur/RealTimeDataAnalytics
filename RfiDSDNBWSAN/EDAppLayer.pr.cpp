/* Process model C++ form file: EDAppLayer.pr.cpp */
/* Portions of this file copyright 1986-2008 by OPNET Technologies, Inc. */



/* This variable carries the header into the object file */
const char EDAppLayer_pr_cpp [] = "MIL_3_Tfile_Hdr_ 145A 30A op_runsim 7 5BD16FD4 5BD16FD4 1 DESKTOP-L8S8FMO Mohammed@Hussein 0 0 none none 0 0 none 0 0 0 0 0 0 0 0 1e80 8                                                                                                                                                                                                                                                                                                                                                                                       ";
#include <string.h>
...
class EDAppLayer_state
	...
	public:
		...
		/* FSM code */
		void EDAppLayer (OP_SIM_CONTEXT_ARG_OPT);
		/* Diagnostic Block */
		void _op_EDAppLayer_diag (OP_SIM_CONTEXT_ARG_OPT);
...
VosT_Obtype EDAppLayer_state::obtype = (VosT_Obtype)OPC_NIL;

#define pr_state_ptr            		((EDAppLayer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))
...
#define FIN_PREAMBLE_DEC	EDAppLayer_state *op_sv_ptr;
#define FIN_PREAMBLE_CODE	\
		op_sv_ptr = ((EDAppLayer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr));
...
// otherwised it will add it to the list
 ...
 /*
	...
	NumberofRFID[c]->weight_kg,
	 qrs,pr,qt  );
	*/
	sprintf (sendThisMsgToKafka, "%d,%d,%d,%d,%d,%d,%d,%d",perdication,
	NumberofRFID[c]->patientID,
	NumberofRFID[c]->age,
	NumberofRFID[c]->gender,
	NumberofRFID[c]->height_cm,
...
extern "C"
{
	VosT_Obtype _op_EDAppLayer_init (int * init_block_ptr);
	VosT_Address _op_EDAppLayer_alloc (VosT_Obtype, int);
	void EDAppLayer (OP_SIM_CONTEXT_ARG_OPT)
		{
		((EDAppLayer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->EDAppLayer (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_EDAppLayer_svar (void *, const char *, void **);

	void _op_EDAppLayer_diag (OP_SIM_CONTEXT_ARG_OPT)
		{
		((EDAppLayer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr))->_op_EDAppLayer_diag (OP_SIM_CONTEXT_PTR_OPT);
		}

	void _op_EDAppLayer_terminate (OP_SIM_CONTEXT_ARG_OPT)
		{
		/* The destructor is the Termination Block */
		delete (EDAppLayer_state *)(OP_SIM_CONTEXT_PTR->_op_mod_state_ptr);
		}
...
void
EDAppLayer_state::EDAppLayer (OP_SIM_CONTEXT_ARG_OPT)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	FIN_MT (EDAppLayer_state::EDAppLayer ());
	try
		{
		/* Temporary Variables */
		Packet*		RfidPkptr;
		...
		FSM_ENTER ("EDAppLayer")

		FSM_BLOCK_SWITCH
			{
			/*---------------------------------------------------------*/
			/** state (init) enter executives **/
			FSM_STATE_ENTER_FORCED_NOLABEL (0, "init", "EDAppLayer [init enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayer [init enter execs]", state0_enter_exec)
				{
				/* Initialization */
				wpan_appl_init ();
				}
			...
			FSM_STATE_EXIT_FORCED (0, "init", "EDAppLayer [init exit execs]")


			/** state (init) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "init", "idle", "tr_1", "EDAppLayer [init -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (idle) enter executives **/
			FSM_STATE_ENTER_UNFORCED (1, "idle", state1_enter_exec, "EDAppLayer [idle enter execs]")

			/** blocking after enter executives of unforced state. **/
			FSM_EXIT (3,"EDAppLayer")


			/** state (idle) exit executives **/
			FSM_STATE_EXIT_UNFORCED (1, "idle", "EDAppLayer [idle exit execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayer [idle exit execs]", state1_exit_exec)
				{
			...
			FSM_PROFILE_SECTION_IN ("EDAppLayer [idle trans conditions]", state1_trans_conds)
			...
			FSM_TRANSIT_SWITCH
				{
				FSM_CASE_TRANSIT (0, 2, state2_enter_exec, ;, "SEND_BE", "", "idle", "send BE", "send best-effort packet", "EDAppLayer [idle -> send BE : SEND_BE / ]")
				FSM_CASE_TRANSIT (1, 3, state3_enter_exec, ;, "SEND_RT", "", "idle", "send RT", "send real-time packet", "EDAppLayer [idle -> send RT : SEND_RT / ]")
				FSM_CASE_TRANSIT (2, 4, state4_enter_exec, ;, "RECEIVE_PCK", "", "idle", "receive pck", "receive packet from NWK", "EDAppLayer [idle -> receive pck : RECEIVE_PCK / ]")
				FSM_CASE_TRANSIT (3, 2, state2_enter_exec, ;, "START_BE_APP", "", "idle", "send BE", "tr_14", "EDAppLayer [idle -> send BE : START_BE_APP / ]")
				FSM_CASE_TRANSIT (4, 5, state5_enter_exec, ;, "ReceiveRFID", "", "idle", "receive RFID", "tr_16", "EDAppLayer [idle -> receive RFID : ReceiveRFID / ]")
				FSM_CASE_TRANSIT (5, 1, state1_enter_exec, ;, "default", "", "idle", "idle", "tr_13", "EDAppLayer [idle -> idle : default / ]")
				}
				/*---------------------------------------------------------*/
			...
			FSM_STATE_ENTER_FORCED (2, "send BE", state2_enter_exec, "EDAppLayer [send BE enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayer [send BE enter execs]", state2_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (2, "send BE", "EDAppLayer [send BE exit execs]")


			/** state (send BE) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send BE", "idle", "tr_6", "EDAppLayer [send BE -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (send RT) enter executives **/
			FSM_STATE_ENTER_FORCED (3, "send RT", state3_enter_exec, "EDAppLayer [send RT enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayer [send RT enter execs]", state3_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (3, "send RT", "EDAppLayer [send RT exit execs]")


			/** state (send RT) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "send RT", "idle", "tr_8", "EDAppLayer [send RT -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (receive pck) enter executives **/
			FSM_STATE_ENTER_FORCED (4, "receive pck", state4_enter_exec, "EDAppLayer [receive pck enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayer [receive pck enter execs]", state4_enter_exec)
				{
			...
			FSM_STATE_EXIT_FORCED (4, "receive pck", "EDAppLayer [receive pck exit execs]")


			/** state (receive pck) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "receive pck", "idle", "tr_10", "EDAppLayer [receive pck -> idle : default / ]")
				/*---------------------------------------------------------*/



			/** state (receive RFID) enter executives **/
			FSM_STATE_ENTER_FORCED (5, "receive RFID", state5_enter_exec, "EDAppLayer [receive RFID enter execs]")
				FSM_PROFILE_SECTION_IN ("EDAppLayer [receive RFID enter execs]", state5_enter_exec)
				{
				//if (RFID_log)
					{
			...
			FSM_STATE_EXIT_FORCED (5, "receive RFID", "EDAppLayer [receive RFID exit execs]")


			/** state (receive RFID) transition processing **/
			FSM_TRANSIT_FORCE (1, state1_enter_exec, ;, "default", "", "receive RFID", "idle", "tr_15", "EDAppLayer [receive RFID -> idle : default / ]")
				/*---------------------------------------------------------*/
		...
		FSM_EXIT (0,"EDAppLayer")
		}
	catch (...)
		{
		Vos_Error_Print (VOSC_ERROR_ABORT,
			(const char *)VOSC_NIL,
			"Unhandled C++ exception in process model (EDAppLayer)",
...
void
EDAppLayer_state::_op_EDAppLayer_diag (OP_SIM_CONTEXT_ARG_OPT)
	{
	/* No Diagnostic Block */
	}

void
EDAppLayer_state::operator delete (void* ptr)
	{
	FIN (EDAppLayer_state::operator delete (ptr));
	Vos_Poolmem_Dealloc (ptr);
	FOUT
	}

EDAppLayer_state::~EDAppLayer_state (void)
	{

	FIN (EDAppLayer_state::~EDAppLayer_state ())
...
void *
EDAppLayer_state::operator new (size_t)
#if defined (VOSD_NEW_BAD_ALLOC)
	...
	FIN_MT (EDAppLayer_state::operator new ());

	new_ptr = Vos_Alloc_Object (EDAppLayer_state::obtype);
#if defined (VOSD_NEW_BAD_ALLOC)
...
EDAppLayer_state::EDAppLayer_state (void) :
		_op_current_block (0)
	{
		...
		_op_current_state = "EDAppLayer [init enter execs]";
#endif
	}

VosT_Obtype
_op_EDAppLayer_init (int * init_block_ptr)
	{
	FIN_MT (_op_EDAppLayer_init (init_block_ptr))

	EDAppLayer_state::obtype = Vos_Define_Object_Prstate ("proc state vars (EDAppLayer)",
		sizeof (EDAppLayer_state));
	*init_block_ptr = 0;

	FRET (EDAppLayer_state::obtype)
	}

VosT_Address
_op_EDAppLayer_alloc (VosT_Obtype, int)
	{
#if !defined (VOSD_NO_FIN)
	int _op_block_origin = 0;
#endif
	EDAppLayer_state * ptr;
	FIN_MT (_op_EDAppLayer_alloc ())

	/* New instance will have FSM handling initialized */
#if defined (VOSD_NEW_BAD_ALLOC)
	try {
		ptr = new EDAppLayer_state;
...
void
_op_EDAppLayer_svar (void * gen_ptr, const char * var_name, void ** var_p_ptr)
	{
	EDAppLayer_state		*prs_ptr;

	FIN_MT (_op_EDAppLayer_svar (gen_ptr, var_name, var_p_ptr))
	...
	prs_ptr = (EDAppLayer_state *)gen_ptr;
